...
/**
 * ************************************************************************
 * 1. KDTree class takes a user-defined Point type as its template parameter
 * 2. A user-defined Point type needs to satisfy following specifications
 *      a) Implementation of operator[] <= accessor to its coordinates
 *      b) Static member variable DIM <= dimension of the Point
 * 3. Passing point cloud to KDTree constructor starts to build k-d tree
 * 4. After building k-d tree, you can use Nearest neighbor search functions
 *
 * example:
 * #include "kdtree.h"
 *
   ...
   private:
    ...
    class BoundedPriorityQueue
    {
     public:
      ...
      {
        auto it = std::find_if(
            std::begin(elements_), std::end(elements_), [&](const T& element) { return Compare()(val, element); });
    ...
    void nnSearchRecursive(const PointT& query, const Node* node, int* guess, double* minDist) const
    {
      ...
      const double diff = mymath::fabs(query[axis] - train[axis]);
      if (diff < *minDist)
        nnSearchRecursive(query, node->next[!dir], guess, minDist);
    }
